...
class RGBTextureFullPipelineBase:
    def __init__(self, pretrain_models=None, pipeline_name='texture_plus',super_resolutions=False, seed=0, speedup_mode=None,add_lora_path=None, add_lora_weights=None, filt_gradient_points=False,filt_large_angle_points=True, enable_rembg=False):
        ...
        self.filt_gradient_points = filt_gradient_points
        self.filt_large_angle_points = filt_large_angle_points
        if self.super_resolutions:
            self.sr_pipeline = TSDSRPipeline()

        ###### build LTM #######
        color_field_vae, color_field_vae_cpu_offload, color_field_vae_sharp_edge = build_ltm(pretrain_models=pretrain_models)
        self.color_field_vae = color_field_vae
        self.color_field_vae_cpu_offload = color_field_vae_cpu_offload
        self.color_field_vae_sharp_edge = color_field_vae_sharp_edge
    ...
    def reproject_and_query_field(self, save_dir, input_mesh_path, input_mv_image_path, camera_info_path, four_or_six=False, flatten=False, method='reproject', inpainting=False):
        ...
        if self.super_resolutions:
            grad_dilation_kernel = 31
        else:
            grad_dilation_kernel = 1
        textured_mesh, reprojected_uv, visable_mask, completed_uv_map = self.inverse_renderer.infer(
            ...
            grad_norm_threhold=0.05,
            ray_normal_angle_threhold=120,
            filt_gradient_points=inpainting and self.filt_gradient_points,
            filt_large_angle_points=inpainting and self.filt_large_angle_points,
            grad_dilation_kernel=grad_dilation_kernel,
        )
        if self.color_field_vae is not None and self.color_field_vae_cpu_offload:
...
class CustomRGBTextureFullPipeline(RGBTextureFullPipeline):
    ...
    def step_2_ablition(self, cache_dir:str, input_image_path:str, input_mesh_path:str, clear_cache=False, *args, **kwargs):
        ...
        if self.super_resolutions:
            self.infer_field(
                os.path.join(cache_dir, 'w_LTM'), os.path.join(cache_dir, 'processed_mesh.obj'), os.path.join(cache_dir, 'sharp_pcd_fps.ply'), os.path.join(cache_dir, 'coarse_pcd_fps.ply'), 
                os.path.join(cache_dir, 'mv_alpha.png'), os.path.join(cache_dir, 'mv_ccm.png'), os.path.join(cache_dir, 'mv_rgb_lr.png'),
                base_or_inpainting=False, test_query_field=False, save_point_clouds=True,
            )
        else:
            self.infer_field(
                os.path.join(cache_dir, 'w_LTM'), os.path.join(cache_dir, 'processed_mesh.obj'), os.path.join(cache_dir, 'sharp_pcd_fps.ply'), os.path.join(cache_dir, 'coarse_pcd_fps.ply'), 
        ...
        self.reproject_and_query_field(os.path.join(cache_dir, 'w_LTM'), os.path.join(cache_dir, 'processed_mesh.obj'), os.path.join(cache_dir, 'mv_rgb.png'), os.path.join(cache_dir, 'camera_info.pth'), inpainting=True)
        self.export_video(os.path.join(cache_dir, 'w_LTM'), os.path.join(cache_dir, 'w_LTM/textured_mesh.glb'), 'textured_mesh.mp4')
        shutil.copy(os.path.join(cache_dir, 'w_LTM/textured_mesh.glb'), os.path.join(cache_dir, 'textured_mesh.glb'))
